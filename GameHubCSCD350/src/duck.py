...
def start_duck(window):
    ...
    for i in range(1, 4):
        ...
        if i < 3:
            ...
            for j in range(1, 5):
                target_images[i - 1].append(pygame.transform.scale(
                    pygame.image.load(f'res/targets/{i}/{j}.png'), (120 - (j * 18), 80 - (j * 12))))
    #track high score to be tracked in a text file to be parsed for the leaderboard
    file = open('duck_high_scores.txt', 'r')
    read_file = file.readlines()
    file.close()
    #diffrent types of learderboard
    best_freeplay = int(read_file[0])
    best_ammo = int(read_file[1])
    best_timed = int(read_file[2])
    #level sounds for future use of multiple levels
    pygame.mixer.init()
    ...
    run = True
    while run:
        #options settings to be accessed via esc
        ...
        #drawing menu screens on to the scren depending on level num
        if menu:
            level = 0
            draw_menu()
        if game_over:
        ...
        #Drawing driffrent enemy patterns for each of the 3 diffrent stages and constantly checks if the duck has been hit
        if level == 1:
            ...
            if (level == 3 and target_boxes == [[], [], [], []]) or (mode == 1 and ammo == 0) or (
                ...
                if mode == 2:
                    if points > best_timed:
                        best_timed = points
                        write_values = True
                #setting game over if stages are finished going to make on going for time leaderboard
                game_over = True
        #writing leader board
        if write_values:
...
#this method draws score to the bottom of the banner while playing called in game loop
def draw_score():
...
#this methos tracks mouse positon and changes the gun image depending on where your mouse is on the screen
def draw_gun():
